...
> images 폴더 안의 logo.png파일(img)
>
> 이미지 크기(style="width: 100px;")
>
> 클릭이 가능한 링크(a)이며, 해당 페이지(02_home.html)로 이동
...
### (6) ul 태그 (navbar-nav) / li 태그 (nav-item) / a 태그 (nav-link active)

> 리스트(Home, Community, Login)는 ul 과 li요소
>
> 클릭이 가능한 링크(a)이며, 해당 페이지(02_home.html, 03_community.html, #)로 이동
>
> Home을 강조 (active)
>
> 양쪽 가로 여백 (mx-2)
...
>
> `for="exampleInputEmail1"`과 `id="exampleInputEmail1"` 값 동일 확인
...
> carousel (div)
>
> `id="carouselExampleIndicators"` id 값 동일 확인
...
### (3) ol 태그 (carousel-indicators) / li 태그 (active)

> 첫번째 페이지 활성화 (active)
>
> `data-bs-target="#carouselExampleIndicators"` id 값 동일 확인
...
### (4) div (carousel-inner) / img 태그

> 자동으로 전환 (data-bs-interval="1000")
>
> 상대경로 이미지 삽입 (img)
>
...
### (5) div (carousel-item active)

> 첫번째 페이지 활성화 (active)

```python
<div class="carousel-item active" data-bs-interval="1000">
  <img src="images\header1.jpg" class="d-block w-100" alt="1"></div>
```

### (6) a 태그 (carousel-control-prev / carousel-control-next)

> `href="#carouselExampleIndicators"` id 값 동일 확인
...
```python
<h1 class="text-center fw-bold dispay-1 my-5">Boxoffice</h1>
```
...
> 이미지(img), 제목, 설명(p)을 포함
>
> 카드 사이즈 너비 높이(style="width: 18rem; height: 33rem;")
>
> p 태그 제목 설명 글자 굵기(fw-bold)
>
> 이미지 사이즈 너비 높이(style="width: 18rem; height: 25rem;")
>
> 상대경로 이미지 삽입 (img)
...
```python
<h1 class="text-center fw-bold dispay-1 my-5">Community</h1>
...
### (3) aside 태그 (col-12 col-lg-2) / ul 태그 (list-group) / li 태그 (list-group-item) / a 태그 (text-decoration-none)

> 각 항목들(Boxoffice, Movies, Genres, Actors)은 ul과 li 요소
>
> 항목들은 클릭이 가능한 링크(a)
>
...
### (4) section 태그 (col-12 col-lg-10) / div 태그 (d-none d-lg-table w-100)
